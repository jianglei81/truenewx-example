<#assign toPage = "${toPage!'$.tnx.pager.toPage'}">
<ul class="pager pull-right" style="margin: 0px;">
    <li class="previous<#if pageNo = 1> text-muted</#if>">
        <a<#if pageNo != 1> <#if (tempHref != "")>href='${tempHref?replace("{pageNo}","${previousPage?c}")}'<#else>onclick="${toPage}(this, ${previousPage?c})"</#if></#if>>&lt;</a>
    </li>
<#if (startPage > 1)>
    <li><a <#if (tempHref != "")>href='${tempHref?replace("{pageNo}","1")}'<#else>onclick="${toPage}(this, 1)"</#if>>1</a></li>
</#if>
<#if (startPage > 2)>
    <li><a <#if (tempHref != "")>href='${tempHref?replace("{pageNo}","${(startPage-1)?c}")}'<#else>onclick="${toPage}(this, ${(startPage-1)?c})"</#if>>&hellip;</a></li>
</#if>
<#list startPage..endPage as i>
    <li<#if pageNo = i> class="active"</#if>>
        <a<#if pageNo != i> <#if (tempHref != "")>href='${tempHref?replace("{pageNo}","${i?c}")}'<#else>onclick="${toPage}(this, ${i?c})"</#if></#if>>${i}</a>
    </li>
</#list>
<#if (endPage  < pageCount-1 || !isCountable)>
    <li><a <#if (tempHref != "")>href='${tempHref?replace("{pageNo}","${(endPage+1)?c}")}'<#else>onclick="${toPage}(this, ${(endPage+1)?c})"</#if>>&hellip;</a></li>
</#if>
<#if (isCountable  && endPage < pageCount)>
    <li><a <#if (tempHref != "")>href='${tempHref?replace("{pageNo}","${pageCount?c}")}'<#else>onclick="${toPage}(this, ${pageCount?c})"</#if>>${pageCount}</a></li>
</#if>
    <li class="next<#if !isMorePage || !isCountable> text-muted</#if>">
        <a<#if isMorePage && isCountable > <#if (tempHref != "")>href='${tempHref?replace("{pageNo}","${nextPage?c}")}'<#else>onclick="${toPage}(this, ${nextPage?c})"</#if></#if>>&gt;</a>
    </li>
<#if pageNoInputtable>
    ...
    <#if goText !="">
        <span><a onclick="${toPage}(this)">${goText}</a></span>
    </#if>
<#else>
