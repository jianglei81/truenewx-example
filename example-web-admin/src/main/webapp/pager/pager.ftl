<div class="row a-pager">
    <ul class="pagination pull-right" style="margin: 0px;">
        <li<#if pageNo = 1> class="text-muted"</#if>>
    ...
    <#if (isCountable  && endPage < pageCount)>
        ...
        <li<#if !isMorePage || !isCountable > class="text-muted"</#if>>
    ...
    <#if isCountable >
        <span>共 <a style="text-decoration: none;">${total?c}</a> 条</span>
    </#if>
    <#if (pageSizeOptions?size !=0 && pageSizeOptions[0]!="") >
        <span>每页</span>
    </#if>
